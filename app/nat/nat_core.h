...
struct nat_cfg {
	...
	char* lan_intf;				/* lan interface id */
	char* wan_intf;				/* wan interface id */
 ...
 * @dpdk_config [in]: DPDK configuration values
 * @return: 0 on success and negative value otherwise
 *
 * @NOTE: In case of failure, all already allocated resource are freed
 */
int nat_init(struct nat_cfg *app_cfg, struct application_dpdk_config *dpdk_config);
 ...
 * @NOTE: In case of failure, all already allocated resource are freed
 */
int nat_pipes_init(struct nat_rule_match *nat_rules, int nat_num_rules, struct nat_cfg *app_cfg, int nb_ports);

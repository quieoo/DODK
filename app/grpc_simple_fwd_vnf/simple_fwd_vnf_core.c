...
static void grpc_port_callback(void *config, void *param){
	struct simple_fwd_config *app_config = (struct simple_fwd_config *) config;

	int port = *(int *) param;
	app_config->grpc_port=port;
	DOCA_LOG_DBG("set grpc port: %d", port);
}
void
register_simple_fwd_params()
{
	...
	struct doca_argp_param grpc_port_param={
		.short_flag="g",
		.long_flag="grpc-port",
		.arguments=NULL,
		.description="the port grpc server listens on",
		.callback=grpc_port_callback,
		.arg_type=DOCA_ARGP_TYPE_INT,
		.is_mandatory=false,
		.is_cli_only=false
	};
	...
	doca_argp_register_param(&grpc_port_param);

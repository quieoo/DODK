...
int manully_add_entry(struct doca_flow_match* match, struct doca_flow_actions* action, struct doca_flow_fwd* fwd, struct doca_flow_error *error){
	struct doca_flow_pipe *pipe=simple_fwd_ins->pipe_notun[0];
	struct doca_flow_monitor monitor = {0};
	struct doca_flow_pipe_entry *entry = doca_flow_pipe_add_entry(0,pipe, match, action, &monitor, fwd, 0, NULL, error);
	if (!entry)
	{
		DOCA_LOG_ERR("failed adding entry to pipe: error=%s, type=%u",
			     error->message, error->type);

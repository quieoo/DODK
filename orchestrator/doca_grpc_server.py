...
CONFIG_FILE_PATH             = './doca_grpc.conf'
...
NAME_TO_PORT = 	{
			...
			'doca_dpi_grpc':		gen_common.eNetworkPort.k_DocaDPI,
		}
...
class DocaOrchestrator(gen_grpc.DocaOrchestrator):
	...
	def Create(self, request, context):
		...
		if program_pid == 0:
			...
			os.execv(process_path, cmd_args)
		...
		for i in range(LIFETIME_CHECK_NUM_RETRIES):
			try:
				stub.HealthCheck(gen_common.HealthCheckReq(), timeout=LIFETIME_CHECK_SLEEP_PERIOD_SECONDS,
					wait_for_ready=True)
				self.clients_db[process_uid] = (stub, program_pid)
	...
	def Destroy(self, request, context):
		...
		client, program_pid = self.clients_db[request.uid]
		...
		# Check if the process needs to be terminated by force
		for i in range(LIFETIME_CHECK_NUM_RETRIES):
			# Wait a bit
			time.sleep(LIFETIME_CHECK_SLEEP_PERIOD_SECONDS)
			# Check if the process is running
			try:
				os.kill(program_pid, 0)
			except OSError:
				break
			...
			rich_status.err_status.is_error = True
			rich_status.err_status.error_msg = f'Failed to destroy {process_uid}!'
			logger.warning(rich_status.err_status.error_msg)
